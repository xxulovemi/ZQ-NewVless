...
function text(c, s = 200) { return new Response(c, { status: s, headers: { 'content-type': 'text/plain; charset=utf-8' } }); }
function json(c, s = 200) { return new Response(JSON.stringify(c), { status: s, headers: { 'content-type': 'application/json; charset=utf-8' } }); }
function b64e(s) { const e = new TextEncoder(), b = e.encode(s); return btoa(String.fromCharCode(...new Uint8Array(b))); }
function randStr() { return Math.random().toString(36).substring(2, 8); }
...
function getPath() { return paths[Math.floor(Math.random() * paths.length)]; }
function getParam(key) { return paramsMap[key][Math.floor(Math.random() * paramsMap[key].length)]; }
...
function parseProxyAddress(proxyStr) {
	...
	if (proxyStr.startsWith('socks://') || proxyStr.startsWith('socks5://')) {
		try {
			...
			return { type: 'socks5', host: url.hostname, port: parseInt(url.port), username: url.username ? decodeURIComponent(url.username) : '', password: url.password ? decodeURIComponent(url.password) : '' };
		} catch { return null; }
	}
...
async function socks5Connect(socket, proxyConfig, targetHost, targetPort, timeoutMs) {
	...
	try {
		...
		if (method === 0x02) {
			...
			auth[0] = 0x01; auth[1] = uBytes.length; auth.set(uBytes, 2);
			auth[2 + uBytes.length] = pBytes.length; auth.set(pBytes, 3 + uBytes.length);
		...
		connect[4] = hostBytes.length; connect.set(hostBytes, 5);
	...
	async fetch(req, env) {
		const getUserConfig = async () => {
			try {
				...
				return merged;
			} catch { return { uuid: 'ef9d104e-ca0e-4202-ba4b-a0afb969c747', domain: '', port: '443', s5: '', proxyIp: '', domains: [], ports: [443] }; }
		};
		...
		const buildVlessUri = (rawPathQuery, uuid, label, workerHost, preferredDomain, port) => {
			return `vless://${uuid}@${preferredDomain}:${port}?encryption=none&security=tls&sni=${workerHost}&type=ws&host=${workerHost}&path=${encodeURIComponent(rawPathQuery)}#${encodeURIComponent(label || preferredDomain)}`;
		};

		const buildVariants = (s5, proxyIp) => {
			const v = [{ label: '仅直连', raw: buildPath('d', 1, null, null) }];
			if (s5) {
				v.push({ label: '仅SOCKS5', raw: buildPath('s', null, s5, null) });
				v.push({ label: '直连+SOCKS5', raw: buildPath('p', 1, s5, null) });
			}
			if (proxyIp) v.push({ label: '直连+ProxyIP', raw: buildPath('p', 1, null, proxyIp) });
			if (s5 && proxyIp) v.push({ label: '直连+SOCKS5+ProxyIP', raw: buildPath('p', 1, s5, proxyIp) });
			return v;
		};
		...
		if (req.headers.get('Upgrade')?.toLowerCase() === 'websocket') {
			...
			if (u.pathname.includes('%3F')) {
				...
				if (queryIndex !== -1) { u.search = decoded.substring(queryIndex); u.pathname = decoded.substring(0, queryIndex); }
			}

			const getParamValue = (keys) => { for (const k of keys) { const v = u.searchParams.get(k); if (v !== null) return v; } return null; };
			...
			if (!proxyConfig && s5Param) {
				const path = s5Param;
				if (path.includes('@')) {
					const [cred, server] = path.split('@'), [user, pass] = cred.split(':'), [host, port] = server.split(':');
					if (host && port) proxyConfig = { type: 'socks5', host, port: +port, username: user, password: pass };
				} else if (path.includes(':')) {
					const [host, port] = path.split(':');
					if (host && port) proxyConfig = { type: 'socks5', host, port: +port, username: '', password: '' };
				}
			}
			...
			const connect2Socks5 = async (proxyConfig, targetHost, targetPort, initialData) => {
				...
				try {
					const { writer, reader } = await socks5Connect(socket, proxyConfig, targetHost, targetPort, 2500);
			...
			new ReadableStream({
				start(ctrl) {
					...
					if (early) {
						try {
							...
							ctrl.enqueue(Uint8Array.from(atob(processed), c => c.charCodeAt(0)).buffer);
						} catch {}
					}
				}
				...
				async write(data) {
					...
					for (let i = 0; i < 16; i++) { if (uuidBytes[i] !== parseInt(expectedUUID.substr(i * 2, 2), 16)) return; }
					...
					pos += 3;
					let addr = '';
					if (type === 1) { addr = `${view.getUint8(pos)}.${view.getUint8(pos + 1)}.${view.getUint8(pos + 2)}.${view.getUint8(pos + 3)}`; pos += 4; }
					else if (type === 2) { const len = view.getUint8(pos++); addr = new TextDecoder().decode(data.slice(pos, pos + len)); pos += len; }
					else if (type === 3) { const ipv6 = []; for (let i = 0; i < 8; i++, pos += 2) ipv6.push(view.getUint16(pos).toString(16)); addr = ipv6.join(':'); }
					else return;
					...
					const connectStreams = async (remoteSocket, webSocket, headerData, retryFunc) => {
						...
						const timeoutId = setTimeout(() => { if (!hasData) dataPromiseResolve(false); }, 40);
					...
					const connectParallel = async () => {
						...
						const tryConnect = async (type) => {
							try {
								...
								if (type === 's5' && proxyConfig) { return await connect2Socks5(proxyConfig, addr, port, payload); }
								if (type === 'proxy') {
									let proxyIp = PROXY_IP;
									if (addr && domainProxyMapping[addr]) proxyIp = domainProxyMapping[addr];
									if (proxyIp) { const [ph, pp = port] = proxyIp.split(':'); return await connectDirect(ph, +pp || port, payload); }
								}
							} catch {}
							return null;
						};
						...
						if (mode === 's5') {
							if (proxyConfig) {
								...
								if (sock) { remote = sock; await connectStreams(sock, ws, header, null); }
							}
							return;
						}
						...
						if (!primary) {
							...
							if (backup) { remote = backup; await connectStreams(backup, ws, header, null); }
							return;
						}
						remote = primary;
						const retryFunc = order.length > 1 ? async () => {
							const backup = await tryNext(1);
							if (backup) { try { primary.close(); } catch {} remote = backup; await connectStreams(backup, ws, header, null); }
						} : null;
		...
		if (url.pathname.startsWith('/api/config/')) {
			...
			if (req.method === 'GET') {
				...
				try {
					...
					if (!incoming.uuid || typeof incoming.uuid !== 'string') return json({ error: 'UUID不能为空' }, 400);
					if (urlUUID !== userConfig.uuid && urlUUID !== incoming.uuid) return json({ error: 'UUID错误，无权访问' }, 403);
					...
					if (env.NewVless) {
						...
						if (normalized.domain && normalized.proxyIp) {
							await env.NewVless.put('domain_proxy_mapping', JSON.stringify({ [normalized.domain]: normalized.proxyIp }));
						}
					}
					return json({ success: true, message: '配置保存成功' });
				} catch { return json({ error: '配置保存失败' }, 500); }
			}
		}
		...
		if (url.pathname.startsWith('/sub')) {
			...
			let 订阅类型 = isSubConverterRequest ? 'mixed' :
			...
			const out = [];
			for (const d of domains) for (const p of ports) for (const v of variants) out.push(buildVlessUri(v.raw, userConfig.uuid, `${v.label} ${d}:${p}`, workerHost, d, p));
			...
			if (订阅类型 === 'mixed') {
				...
				try {
					const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter for ' + 订阅类型, 'Accept': '*/*' } });
					if (response.ok) {
